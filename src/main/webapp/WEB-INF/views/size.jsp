    ...
    <table class="table" style="background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        ...
        <c:forEach var="sz" items="${listSize}">
            <tr>
                ...
                <td>${sz.ngaySua}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/size/edit/${sz.id}" class="btn btn-primary btn-sm">Sửa</a>
                    <a href="${pageContext.request.contextPath}/size/delete/${sz.id}" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc muốn xóa danh mục này?');">Xóa</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
