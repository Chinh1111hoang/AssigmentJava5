...
<div class="d-flex justify-content-center mt-3">
    <div class="col-md-8 account-info" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); height: 450px;">
        <h2 class="text-center">Tạo danh mục sản phẩm</h2>
        <form action="${pageContext.request.contextPath}/danhmuc/save" method="post">
            <input type="hidden" name="id" value="${danhMuc.id}">
            <div class="mb-3">
                <label class="form-label">Mã danh mục:</label>
                <input type="text" name="maDanhMuc" class="form-control" value="${danhMuc.maDanhMuc}" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="trangThai" value="UnActive" ${danhMuc.trangThai == 'UnActive' ? 'checked' : ''}>
    ...
    <table class="table" style="background-color: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        ...
        <c:forEach var="dm" items="${listDM}">
            <tr>
                ...
                <td>${dm.ngaySua}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/danhmuc/edit/${dm.id}" class="btn btn-primary btn-sm">Sửa</a>
                    <a href="${pageContext.request.contextPath}/danhmuc/delete/${dm.id}" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc muốn xóa danh mục này?');">Xóa</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
